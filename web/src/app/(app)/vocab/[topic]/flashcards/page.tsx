 ...
 *  bản theo bài ở `/learn/[phase]/[lesson]/vocab/flashcards`, vốn lấy trọn
 *  từ vựng của một bài. */
export default async function TopicGamePage({
  params,
}: {
  ...
  return (
    <div className="lg:fixed lg:inset-0 lg:z-50 lg:flex lg:overflow-y-auto lg:bg-background">
      <div className="mx-auto flex w-full max-w-2xl flex-col px-4 py-6 lg:py-8">
        <GameTopBar backHref={backHref} label={`${session.emoji} ${session.nameVi} · Thẻ ghi nhớ`} />
        ...
        <div className="flex flex-1 flex-col justify-center pb-10">
          {session.words.length === 0 ? (
            ...
            <Flashcards words={session.words.map((w) => ({ ...w, groupName: session.nameVi }))} backHref={backHref} linkComponent={Link} />
          )}
        </div>
      </div>
    </div>
